...
time_o = [(i-1) for i in range(len(actual1))]
time_a = [(i-0) for i in range(len(actual1))]
...
ax.plot(time_a[:len(allowed1)], allowed1, marker='.', color='none')  # invisible markers
ax.fill_between(time_a[:len(predicted1)], predicted1, allowed1, color='#c4daec', alpha=0.5, label="Allowed Margin")
