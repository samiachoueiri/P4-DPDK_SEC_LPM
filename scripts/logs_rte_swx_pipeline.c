		    ...
		    struct instruction_data *data __rte_unused)
{
	...
	if(strcmp(src,"m.pna_main_input_metadata_timestamp1") == 0) {
		instr->type = INSTR_MOV_TS1;
		instr->mov.dst.struct_id = (uint8_t)dst_struct_id;
		instr->mov.dst.n_bits = fdst->n_bits;
		instr->mov.dst.offset = fdst->offset / 8;
		//instr->mov.src_val = 123;
...
static inline void
instr_mov_ts1_exec(struct rte_swx_pipeline *p)
{
	struct thread *t = &p->threads[p->thread_id];
	struct instruction *ip = t->ip;

	__instr_mov_ts1_exec(p, t, ip);
...
static instr_exec_t instruction_table[] = {
	...
	[INSTR_MOV_TS1] = instr_mov_ts1_exec,
...
instr_type_to_name(struct instruction *instr)
{
	...
	case INSTR_MOV_TS1: return "INSTR_MOV_TS1";
...
static instruction_export_t export_table[] = {
	...
	[INSTR_MOV_TS1] = instr_mov_export,
...
instr_type_to_func(struct instruction *instr)
{
	...
	case INSTR_MOV_TS1: return "__instr_mov_ts1_exec";
